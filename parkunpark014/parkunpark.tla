...
\* Implementation of default-park from tokio-threadpool 0.1.4
--algorithm ParkUnpark {
    ...
    variables old;
    {
        unpark_start: while (TRUE) {
            ...
            \* Transition SLEEP -> NOTIFY
            if (state = SLEEP) {
                state := NOTIFY;
            } else {
                \* All done - no need to wake
                goto unpark_unlock;
            };
...
park_unlock == /\ pc["parker"] = "park_unlock"
               /\ Assert(mx = LOCKED, 
                         "Failure of assertion at line 65, column 26.")
               /\ mx' = FREE
               /\ pc' = [pc EXCEPT !["parker"] = "park_continue"]
               /\ UNCHANGED << state, cv, old_, old >>
...
unpark_lock(self) == /\ pc[self] = "unpark_lock"
                     ...
                     /\ IF state = SLEEP
                           THEN /\ state' = NOTIFY
                                /\ pc' = [pc EXCEPT ![self] = "unpark_wake"]
                           ELSE /\ pc' = [pc EXCEPT ![self] = "unpark_unlock"]
                                /\ state' = state
                     /\ UNCHANGED << cv, old_, old >>
...
Unparker(self) == unpark_start(self) \/ unpark_precheck(self)
                     \/ unpark_lock(self) \/ unpark_wake(self)
                     \/ unpark_unlock(self)
...
\* Last modified Tue Aug 07 16:26:47 PDT 2018 by jsgf
