...
\* Implementation of default_park from https://github.com/tokio-rs/tokio/pull/528
--algorithm ParkUnpark {
    ...
    variables old;
    {
        park_start: while (TRUE) {
            ...
            if (state = NOTIFY) {
                \* Atomic NOTIFY -> IDLE transition
                state := IDLE;
            } else {
                \* Spurious wakeup
                goto park_sleep_release;
            };
...
***************************************************************************)
\* BEGIN TRANSLATION
\* Process variable old of process Parker at line 17 col 15 changed to old_
...
park_sleep_release == /\ pc["parker"] = "park_sleep_release"
                      /\ Assert(mx = LOCKED, 
                                "Failure of assertion at line 54, column 33.")
                      /\ mx' = FREE
...
park_unlock == /\ pc["parker"] = "park_unlock"
               /\ Assert(mx = LOCKED, 
                         "Failure of assertion at line 70, column 26.")
               /\ mx' = FREE
               /\ pc' = [pc EXCEPT !["parker"] = "park_continue"]
               /\ UNCHANGED << state, cv, old_, old >>
...
Parker == park_start \/ park_precheck \/ park_lock \/ park_sleep
             ...
             \/ park_resume \/ park_unlock \/ park_continue
...
inv_start == /\ pc["invariants"] = "inv_start"
             /\ \/ /\ Assert(state \in { IDLE, NOTIFY, SLEEP }, 
                             "Failure of assertion at line 112, column 20.")
                \/ /\ Assert(mx \in { FREE, LOCKED }, 
                             "Failure of assertion at line 113, column 16.")
                \/ /\ Assert(cv \in { 0, 1 }, 
                             "Failure of assertion at line 114, column 16.")
...
\* Last modified Wed Aug 08 12:18:57 PDT 2018 by jsgf
